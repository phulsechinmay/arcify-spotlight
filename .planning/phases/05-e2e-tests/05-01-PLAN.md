...
files_modified:
  - overlay.js
  - e2e/setup.js
  - e2e/tests/spotlight.e2e.test.js
autonomous: true
...
must_haves:
  ...
  artifacts:
    ...
    - path: "e2e/tests/spotlight.e2e.test.js"
      provides: "3-5 E2E tests covering critical user flows"
      min_lines: 100
  key_links:
    - from: "e2e/tests/spotlight.e2e.test.js"
      to: "e2e/setup.js"
...
<task type="auto">
  ...
  <files>overlay.js, e2e/setup.js</files>
  <action>
...
3. Grep confirms setup exports: `grep -n "export.*function" e2e/setup.js`
  </verify>
  <done>
- overlay.js has 5 data-testid attributes on key elements
- e2e/setup.js exports: launchBrowserWithExtension, closeBrowser, getServiceWorker, waitForExtension, openNewTabPage
  </done>
</task>

<task type="auto">
  <name>Task 2: Create E2E tests for critical user flows</name>
  <files>e2e/tests/spotlight.e2e.test.js</files>
  <action>
Create e2e/tests/ directory and spotlight.e2e.test.js with 3-4 focused E2E tests.
...
3. All tests pass or have clear failure reason related to environment (not code bugs)
  ...
  <done>
- e2e/tests/spotlight.e2e.test.js exists with 3-4 tests
...
3. **Test count verification:**
   ...
   grep -c "it\(" e2e/tests/spotlight.e2e.test.js
